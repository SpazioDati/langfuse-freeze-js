...
export class LangfuseBacked extends LangfuseClient {
  ...
  constructor(params?: LangfuseClientParams) {
    ...
    try {
      ...
      throw new Error(`Prompts backup at ${backupPath} contains invalid JSON: ${e}`);
    }
    ...
    this.prompt = new Proxy(originalPrompt, {
      get(target, prop) {
        ...
        return (name: string, options?: Parameters<PromptManager["get"]>[1]): Promise<TextPromptClient | ChatPromptClient> => {
